...
st.set_page_config(
    page_title='[Sleep-2-Learn] Model Performance - Sleep Stage Classifier', layout='wide')


def main():
    st.set_option('deprecation.showPyplotGlobalUse', False)

    st.title("Sleep-2-Learn : Model Performance Test")
    st.sidebar.title("ML Classification")
    st.markdown("Test classifiers performance")

    @st.cache(persist=True)
    def load_data():
        data = pd.read_csv("./46343.csv")
    ...
    if classifier == 'Logistic Regression':
        ...
        max_iter = st.sidebar.slider(
            "Maximum number of iterations", 100, 1000, key='max_iter')
    ...
    if st.sidebar.checkbox("Show raw data", False):
        st.subheader("Test Dataset (Subject 46343)")
        st.write(df)
